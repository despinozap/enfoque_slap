...
class EntregasController extends Controller
{
            ...
            if($user->role->hasRoutepermission('centrosdistribucion entregas_store'))
            {
                if($centrodistribucion = Sucursal::where('id', $centrodistribucion_id)->where('type', 'centro')->first())
                {
                    // Ocs with Partes pending for deliver GET OCPARTE
                    {
                        ...
                        $ocList = $ocParteList->reduce(function($carry, $ocParte)
                            {
            ...
            if($user->role->hasRoutepermission('centrosdistribucion entregas_store'))
            {
                if($centrodistribucion = Sucursal::where('id', $centrodistribucion_id)->where('type', 'centro')->first())
                {
                    if($oc = Oc::select('ocs.*')
                            ->join('cotizaciones', 'cotizaciones.id', '=', 'ocs.cotizacion_id')
                            ->join('solicitudes', 'solicitudes.id', '=', 'cotizaciones.solicitud_id')
                            ->where('solicitudes.sucursal_id', '=', $centrodistribucion->id) // Oc belongs to the Sucursal (centro)
                    ...
                    )
                    {
                        ...
                        $queuePartes = $oc->partes->reduce(function($carry, $parte) use($centrodistribucion)
                            {
                                // Get the stock cantidad in Sucursal (centro)
                    ...
                    else
                    {
                        $response = HelpController::buildResponse(
                            412,
                            'La OC no existe en el centro de distribucion',
                            null
                        );
                    }                    
                }   
    ...
    public function show_centrodistribucion($centrodistribucion_id, $id)
    {
        ...
        {
            $user = Auth::user();
            if($user->role->hasRoutepermission('centrosdistribucion entregas_show'))
            {
                if($centrodistribucion = Sucursal::where('id', $centrodistribucion_id)->where('type', 'centro')->first())
                {
                    if($entrega = $centrodistribucion->entregas->find($id))
                    {
                        $centrodistribucion->makeHidden([
                            'created_at', 
                            'updated_at'
    ...
    public function update_prepare_centrodistribucion($centrodistribucion_id, $id)
    {
        ...
        {
            $user = Auth::user();
            if($user->role->hasRoutepermission('centrosdistribucion entregas_update'))
            {
                if($centrodistribucion = Sucursal::where('id', $centrodistribucion_id)->where('type', 'centro')->first())
                {
                    if($entrega = $centrodistribucion->entregas->find($id))
                                ...
                                foreach($request->partes as $parteRequest)
                                {
                                    ...
                                    )
                                    {
                                        ...
                                        if($cantidadEntregas <= $ocParteEntrega->ocparte->cantidad)
                                        {
                                            ...
                                            // Calc stock for Parte using cantidad in Recepciones - cantidad in Entregas - cantidad in Despachos + cantidad in this Entrega - new cantidad
                                            $cantidadStock = $ocParteEntrega->ocparte->parte->getCantidadRecepcionado($centrodistribucion) - $ocParteEntrega->ocparte->parte->getCantidadEntregado($centrodistribucion) - $ocParteEntrega->ocparte->parte->getCantidadDespachado($centrodistribucion) + $ocParteEntrega->cantidad - $parteRequest['cantidad'];
                                            if($cantidadStock >= 0)
                                            {
                                            ...
                                            {
                                                // If the delivered parts are more than in stock in Sucursal (centro)
                                                $response = HelpController::buildResponse(
                                                    409,
                                    ...
                                    )
                                    {
                                        ...
                                        if($cantidadEntregas <= $ocParte->cantidad)
                                        {
                                            ...
                                            // Calc stock for Parte using cantidad in Recepciones - cantidad in Entregas - cantidad in Despachos - new cantidad
                                            $cantidadStock = $ocParte->parte->getCantidadRecepcionado($centrodistribucion) - $ocParte->parte->getCantidadEntregado($centrodistribucion) - $ocParte->parte->getCantidadDespachado($centrodistribucion) - $parteRequest['cantidad'];
                                            if($cantidadStock >= 0)
                                            {
    ...
    public function destroy_centrodistribucion(Request $request, $centrodistribucion_id, $id)
    {
        ...
        {
            $user = Auth::user();
            if($user->role->hasRoutepermission('centrosdistribucion entregas_destroy'))
            {
                if($centrodistribucion = Sucursal::where('id', $centrodistribucion_id)->where('type', 'centro')->first())
                {
                    if($entrega = $centrodistribucion->entregas->find($id))
                                ...
                                foreach($request->partes as $parteRequest)
                                {
                                    ...
                                    )
                                    {
                                        ...
                                        if($cantidadEntregas <= $ocParte->cantidad)
                                        {
                                            ...
                                            // Calc stock for Parte using cantidad in Recepciones - cantidad in Entregas - new cantidad
